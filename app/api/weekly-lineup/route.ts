...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (request.nextUrl.searchParams.get("estimates") === "nflverse") {
      ...
      const result = await generateNflverseWeeklyProjections({ season: snapshot.season, week, profile, playerIds: snapshot.players.map((p) => p.id), now: Date.now(), includeConditionalEstimates: request.nextUrl.searchParams.get("conditional") === "active-at-kickoff", includeExperimentalEstimates: request.nextUrl.searchParams.get("experimental") === "coverage-baselines" });
