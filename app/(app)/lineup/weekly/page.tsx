...
function ScopedWeeklyLineup({ storageScope }: { storageScope: string | null }) {
  ...
  const input = useMemo(() => snapshot === null ? null : {
    ...selectWeeklyConditionalEstimates(snapshot, includeConditional, now), source: snapshot.model ? `${snapshot.model.source}; manual overrides: ${source}` : source,
    projectionUpdatedAt: publication === "" ? null : Date.parse(publication),
  }, [snapshot, source, publication, includeConditional, now]);
