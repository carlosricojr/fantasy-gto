...
export default function WeeklyLineupPage() {
  ...
  async function refresh() {
    ...
    try {
      ...
      const values = new Map(prior?.players.map((p) => [p.id, p.projectedPoints]));
      setSnapshot(sameContext ? { ...incoming, retrievedAt: prior.retrievedAt, players: incoming.players.map((p) => ({ ...p, projectedPoints: values.get(p.id) ?? null })) } : incoming);
      if (!sameContext) setPublication("");
      setNow(Date.now());
  ...
  return <PageShell title="Weekly lineup" subtitle="Your current starters, your league scoring, and kickoff locks.">
    <p className="mb-6 text-sm text-muted-foreground">Import your Sleeper roster, then enter expected points calculated for the scoring rules shown below. This planner maximizes those supplied estimates; it cannot guarantee the highest actual score. It does not submit a lineup to Sleeper.</p>
    ...
    <Button className="mt-3" onClick={refresh} disabled={busy || !leagueId || !ownerId}>{busy ? "Importing…" : snapshot ? "Refresh current roster" : "Import current roster"}</Button>
    <p className="mt-2 text-xs text-muted-foreground">Reads public league and player data from Sleeper and the schedule from nflverse. No automatic third-party projection download.</p>
    ...
    {snapshot && <div className="mt-8 space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm">Projection source<Input value={source} onChange={(e) => setSource(e.target.value)} /></label>
        <label className="text-sm">Source publication time, if known<Input type="datetime-local" value={publication} onChange={(e) => setPublication(e.target.value)} /><span className="text-xs text-muted-foreground">Your local time. Leave blank if unknown; do not use retrieval time.</span></label>
      </div>
      ...
      <div className="relative overflow-x-auto rounded-lg border"><table className="w-full text-left text-sm"><thead className="border-b bg-muted/40"><tr><th className="p-3">Player / current slot</th><th className="p-3">Status / kickoff</th><th className="p-3">Expected points</th></tr></thead><tbody>{snapshot.players.map((player) => <tr key={player.id} className="border-b last:border-0">
        ...
        <td className="p-3"><Input className="min-w-24" type="number" step="0.01" aria-label={`Expected points for ${player.name}`} value={player.projectedPoints ?? ""} onChange={(e) => {
          ...
          setSnapshot((old) => old === null ? null : { ...old, players: old.players.map((p) => p.id === player.id ? { ...p, projectedPoints: value === "" ? null : Number(value) } : p) });
        }} /></td>
      </tr>)}</tbody></table></div>
      ...
      {plan && <section aria-live="polite" className="rounded-lg border p-4">
        ...
        {plan.status !== "blocked" && <>
          <p className="mt-4 text-sm">{plan.excludedSlotIds.length ? "Compared positions" : "Projected lineup"}: {plan.projectedPoints?.toFixed(2)} points{plan.gain === null ? "" : ` · ${plan.gain >= 0 ? "+" : ""}${plan.gain.toFixed(2)} vs current starters`}. Estimates are rounded to cents for assignment.</p>
