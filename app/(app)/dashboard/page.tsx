...
export default function DashboardPage() {
  ...
  const saveConnection = useAction(api.sleeperConnections.save);
  const removeConnection = useMutation(api.sleeperConnections.remove);
  ...
  const atFreeLimit =
    planKnown && me.plan === "free" && (leagues?.length ?? 0) + (connections?.length ?? 0) >= freeLeagues;
