...
import { weeklyLineupPrefill, type SleeperConnection } from "@/lib/nfl/sleeper-connection";
...
export default function WaiversPage() {
  return <PrivateDataGate title="Waiver comparison"><ConnectedWaiverPlanner /></PrivateDataGate>;
}

function ConnectedWaiverPlanner() {
  const me = useQuery(api.users.me, {});
  const connections = useQuery(api.sleeperConnections.list, {});
  if (me === undefined || connections === undefined) return <PageShell title="Waiver comparison"><p role="status">Loading your saved teams…</p></PageShell>;
  if (!can(me.entitlements, "waiver_comparison")) return <PageShell title="Waiver comparison"><p>Waiver comparison requires Pro access. You are signed in; no additional sign-in is needed.</p><Link href="/pricing" className="mt-4 inline-block underline">Plan details</Link></PageShell>;
  return <WaiverPlanner connections={connections} />;
}

/** View exercised with synthetic local fixtures as well as the authenticated route. */
export function WaiverPlanner({ connections = [] }: { connections?: readonly SleeperConnection[] }) {
  ...
  useEffect(() => {
    ...
    setLeagueId(prefill.leagueId); setOwnerId(prefill.ownerId); setWeek(prefill.week);
    const tick = () => setNow(Date.now());
    tick();
    const timer = window.setInterval(tick, 1000);
    window.addEventListener("focus", tick);
    return () => { generation.current += 1; window.clearInterval(timer); window.removeEventListener("focus", tick); };
  }, []);
  function clearContext() { generation.current += 1; setBusy(false); setPool(null); setComparisonData(null); setSelected([]); setError(""); setQuery(""); setIncludeConditional(false); setCompareAvailable(false); }
  function selectConnection(connection: SleeperConnection, selectedWeek?: number) {
    clearContext(); setLeagueId(connection.leagueId); setOwnerId(connection.ownerId); setWeek(selectedWeek === undefined ? "" : String(selectedWeek));
  }
  async function refresh(compare: boolean) {
    ...
    try {
      ...
      if (selectedWeek === "") {
        ...
        if (!lookup.ok) throw new Error(body.error ?? "Could not resolve the current week.");
        if (!Number.isInteger(body.week) || body.week < 1 || body.week > 18 || !Array.isArray(body.connections) || !body.connections.some((connection: SleeperConnection) => connection.leagueId === leagueId && connection.ownerId === ownerId)) throw new Error("Current-week lookup did not confirm this exact league and manager.");
        selectedWeek = String(body.week); setWeek(selectedWeek);
      }
      ...
      if (incoming.roster.leagueId !== leagueId || incoming.roster.ownerId !== ownerId || incoming.roster.week !== Number(selectedWeek)) throw new Error("Response does not match the requested league, manager and week.");
      setPool(incoming);
  ...
  return <PageShell title="Waiver comparison" subtitle="One week. One addition and one drop. No transactions submitted.">
    ...
    <p className="mb-4 text-sm">Pro waiver-comparison access · <Link className="underline" href="/pricing">Plan details</Link>. FAAB remains unimplemented.</p>
    {connections.length > 0 && <section className="mb-4 rounded-lg border p-4"><h2 className="font-semibold">Your saved Sleeper teams</h2><div className="mt-2 flex flex-wrap gap-2">{connections.map((connection) => <Button key={`${connection.leagueId}:${connection.ownerId}`} variant="outline" disabled={busy} onClick={() => selectConnection(connection)}>{connection.leagueName} · {connection.username}</Button>)}</div><p className="mt-2 text-xs text-muted-foreground">Selecting a team only fills the form. Loading verifies current membership and resolves Sleeper’s current week.</p></section>}
    <details className="mb-4"><summary className="cursor-pointer text-sm">Find another team by Sleeper username or league link</summary><div className="mt-2"><SleeperConnectionFinder onSelect={selectConnection} /></div></details>
    <div className="grid gap-3 sm:grid-cols-3">
      ...
      <label className="text-sm">Current week<Input value={week} disabled={busy} type="number" min={1} max={18} placeholder="Resolve current week" onChange={(e) => { clearContext(); setWeek(e.target.value); }} /></label>
    </div>
    ...
    {pool && <div className="mt-6 space-y-5">
      <section className="rounded-lg border p-4 text-sm">
        ...
        <p className="mt-1 text-muted-foreground">{pool.availableCount} unrostered candidates with current-week NFL roster evidence; {pool.directoryExcludedCount} other unrostered directory entries are malformed, unavailable or ineligible. Directory designations can be cached for a day. Unrostered does not mean immediately claimable: verify waiver clearance, roster restrictions, final active lists and your intended drop in Sleeper.</p>
        <p className="mt-2 text-muted-foreground">NFL membership exclusions: {pool.rosterEvidence.excluded.missing} missing, {pool.rosterEvidence.excluded.inactive} inactive, {pool.rosterEvidence.excluded.conflicting} conflicting. <a href={pool.rosterEvidence.sourceUrl} className="underline">Roster source</a>: {pool.rosterEvidence.reportedTeams.length}/32 teams reported for this week; publication time unknown. This is the complete filtered pool, not complete NFL coverage or injury clearance. Missing evidence is never treated as active.</p>
