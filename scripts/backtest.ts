...
import { impliedTeamTotal, buildDefenseFactors, projectPlayer } from "@/lib/nfl/model/project";
...
async function main(): Promise<void> {
  ...
  // Each team's own average implied total, per season. The Vegas adjustment is measured
  // against this, not the league average — see docs/model-validation.md.
  ...
  const teamMeanTotal = new Map<string, number>();
  for (const [key, values] of teamTotals) teamMeanTotal.set(key, mean(values));
        ...
        const projection = projectPlayer({
          ...
          game: {
            ...
            teamMeanImpliedTotal: team
              ? (teamMeanTotal.get(`${week.period.season}:${team}`) ?? null)
              : null,
          },
          scoring: PPR,
