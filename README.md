...
| Projection floor and ceiling, and the weekly spread the draft simulation draws from | Empirical 10th/90th percentiles of actual/projected, measured per position after calibration, **under PPR**. Other rulesets carry a visible caveat. The four projected positions are the only ones that reach a displayed floor or ceiling; the kicker and defence entries are never rendered, and their only effect is on the weekly draws behind a championship probability. Kicker and defence are projected by nothing, so their bands cannot be measured this way and are not: they come from historical weekly scoring against the entity's own prior-season points per game, reproduced by `pnpm verify-sources` and recorded in [`docs/data-sources.md`](docs/data-sources.md). Both are ruleset-independent, since the three presets score K and D/ST identically. The defence band is the one number here that is a *fit* rather than a quantile — an eighth of D/ST weeks score nothing or less, so its empirical 10th percentile is 0.000 and no multiplicative band can carry it; the shipped pair is the closest one that reproduces the measured expectation of a weekly maximum, which is the only thing the season simulation reads a band for. Measuring these bands does not make either position projected, and the market-only labels on their rows are unchanged. |
...
| Weekly lineup with imported starters and kickoff locks | `/lineup/weekly` imports the current Sleeper roster and takes user-entered weekly expected points under its exact scoring identity. `planWeeklyLineup` maximizes those estimates in cents, preserves started starters and bench players, and uses later kickoffs in flexible slots only among equal-point alternatives. Missing values and stale roster/projection inputs block advice; explicit unpriced K/DST exclusions produce scoped totals, never a full-roster optimum. Independent brute-force tests verify assignment. See [`docs/weekly-lineup.md`](docs/weekly-lineup.md) for source and freshness limits. |
...
| Draft recommendations ranked by championship probability | Simulated season in `lib/core/season-sim.ts`; the panel calls this a **simulated title chance**, conditional on the pick, the recorded board, and the default policy completing the unfilled draft. It is not a calibrated real-world forecast. A uniform pre-draft `1 / teams` reference is shown only to explain the scale; it assumes identical teams and is neither the comparison nor a target. The displayed `±` is one standard error, `sqrt(p(1-p)/scenarios)`, from the finite scenario count shown in the panel — not all uncertainty. A candidate&apos;s comparison with the displayed leader instead uses the paired 95% range from `pairedOutcomeComparison`; a range spanning zero gets the visible viable-alternative/tied label while the numerical ordering remains by title chance. The `pp` figure compares the candidate with the default policy&apos;s available pick under the same completion rule, not the equal-chance reference. A recorded player outside the valued pool — for example, an unavailable player or one otherwise lacking an active valuation — is simulated at the lowest current value for that player&apos;s position when one is available (otherwise a fallback), not at a personal projection; if that player is on **your** roster, the panel names them and marks the title estimate incomplete rather than pick-ready. **No ranking edge over ADP is claimed** — measured out-of-sample, the market ranks players better than our model. See [`docs/draft-validation.md`](docs/draft-validation.md). |
...
| A market projection from an exact flat curve is shown as a position mean with no within-position ADP ordering | `marketValueBasis` in `lib/nfl/draft/value.ts` reads the exact selected curve: `slope === 0` is `position-mean` (or `pooled-mean` for a fallback), while every negative slope — however near flat — remains `adp-ordered`. `convex/ingest.ts` stores that fact on each newly built row; older rows without the field remain unknown rather than being classified from equal rounded values. `valueBasis` carries the mean-only state into the row badge and explanation, and the Market line in `player-detail.tsx` says that the curve is flat, the number is the historical mean, and it carries no within-position ADP ordering. A player with no market price is instead told that no market estimate is available. The fit itself is unchanged by this disclosure. |
...
| The measured K and D/ST bands do not move any of the nine draft properties | `pnpm draft-mock -- --current-bands` replays both harness modes with every row re-banded from `OUTCOME_QUANTILES` instead of the frozen fixture's, and enforces the same nine checks. Both finish 9/9, on a different sixteen players from the default run. The fixture is the #88 audit's record and is deliberately never regenerated, so the default run replays the August-2026 bands; this flag is how the gap between that board and the one production builds is measured rather than assumed. The banner's count of re-banded rows is computed at run time from the same rewriting the replay uses, so it is output rather than a claim checked in anywhere. |
| A kicker or defence is not recommended before the round the market drafts him in, and never a second one before the last two rounds | `applyStreamableDiscipline` is keyed to `UNPROJECTED_POSITIONS` (K/D-ST), not waiver cover. `waiverWireCover` separately derives QB coverage from fantasy demand against 32 current NFL starters: 1.00 in the audited 10-team 1-QB league and 0.60 in a 10-team SUPERFLEX/2QB league. Thus coverage changes replacement valuation without silently extending market-only K/D-ST discipline to modeled QBs. Regression-locked as checks (b), (d), and (g)–(i) of `pnpm draft-mock`, in both modes. |
