...
- **One season cannot detect an improvement smaller than about 2%.** The minimum detectable
  ...
  smaller is measured there and not on one season; the measured figures are in
  [`docs/model-validation.md`](docs/model-validation.md), which `pnpm backtest` reproduces.
