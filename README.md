...
| Weekly injury reports are pre-kickoff | `pnpm verify-sources`. Every 2024 regular-season row joins to its game's kickoff, and 5,953 of 5,954 (**99.98%**) were last modified before it, median 47 hours before — Friday for a Sunday game, which is when the NFL mandates the final report. **2025 carries no `date_modified`, so this cannot be verified on the holdout**; the claim there is inference, not verification, and [`docs/data-sources.md`](docs/data-sources.md) says so. |
