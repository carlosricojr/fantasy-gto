...
| A player with no market price is not recommended in the first six rounds | `applyMarketGate` in `lib/core/draft-policy.ts`: through round 6 the recommendation shortlist admits only players the board prices with an `adp`; a market-absent player's model projection still appears on the board under its "no market price" badge, and past round 6 it may lead again. The measured reason is the #88 audit — the panel recommended market-absent players at picks 2.06 and 6.06 on the model's number alone, and `docs/draft-validation.md` records that the model's ranking does not beat the market's. The Sleeper players dump feeds a coverage signal for exactly those rows (`search_rank`, depth chart; measured in `docs/data-sources.md`) — it is a search-relevance ordering, **not a market price**: it never enters the ADP curve fit (`market-awareness.test.ts` walks every caller and asserts it), is never blended into a valuation, and no label presents it as a price. Regression-locked as check (a) of `pnpm draft-mock`. |
