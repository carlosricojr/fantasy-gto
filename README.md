...
| The season a draft is simulated over: regular-season weeks and the playoff rounds | Derived by `fantasySeasonWeeks` from the two settings the setup screen asks for, the championship week and the size of the playoff field. Every week from 1 to the final belongs to exactly one half, and the bracket is exactly as long as the field needs — asserted in `season-sim.test.ts` for every combination the interface offers. This used to be two literals applied to every league, which is recorded with what it cost in [`docs/draft-validation.md`](docs/draft-validation.md). Byes are priced by the week they land in, so this is not presentational: a bye in a round every qualifier plays takes at least 30% off a team's championship probability where the same bye in the regular season takes under 20%, which is the pair `season-sim.test.ts` asserts. |
