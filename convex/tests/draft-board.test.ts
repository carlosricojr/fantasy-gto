...
describe("draft board publishing", () => {
  ...
  it("keeps serving the previous board whole when a rebuild dies mid-write", async () => {
    ...
    expect(await t.query(api.draft.boardFreshness, shape)).toEqual({ computedAt: 1_000, adpSourceTeams: 12 });
  });
  ...
  it("swaps the whole board at once when the rebuild finishes", async () => {
    ...
    expect(await t.query(api.draft.boardFreshness, shape)).toEqual({ computedAt: 2_000, adpSourceTeams: 12 });
  });
  ...
  it("does not let a stale run that finishes late take the current board with it", async () => {
    ...
    expect(await t.query(api.draft.boardFreshness, shape)).toEqual({ computedAt: 5_000, adpSourceTeams: 12 });
  ...
  it("serves the newest run if a second pointer row ever appears", async () => {
    ...
    expect(await t.query(api.draft.boardFreshness, shape)).toEqual({
      computedAt: 2_000,
      adpSourceTeams: 12,
    });
    expect((await t.query(api.draft.board, shape)).map((r) => r.playerId)).toEqual(["new"]);
  ...
  it("does not let a rebuild of one league size disturb another", async () => {
    ...
    expect(await t.query(api.draft.boardFreshness, ten)).toEqual({ computedAt: 1_000, adpSourceTeams: 12 });
