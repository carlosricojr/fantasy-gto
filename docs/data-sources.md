...
as the source of `scoring_settings`. The league adapter now parses supported coefficients
into an exact custom profile, rather than mistaking the draft metadata's reception-format
label for complete rules. League season settings are imported separately.
Coefficient and unsupported-bonus tests use explicitly synthetic payloads; they do not
by themselves verify any particular live league. A missing league response blocks
import. Custom profiles require their own published board, including K/DST scoring; they
cannot silently read a preset board. Standalone mocks retain preset-label behavior.
...
Public and unauthenticated. Verified by direct request on 2026-09-08 for 2024 week 1: the response
...
rejects a response that carries a different week, and
