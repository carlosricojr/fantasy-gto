...
| --- | --- | --- |
| September 9, 2026 — explicit experimental week-1 coverage alternatives | `pnpm backtest` on base `5d0c484` and candidate with identical frozen source cache | Both reproduce development MAE 5.8818 (26,837 rows) and tuning 5.7709 (9,063 rows). The earlier fresh-source 9,069-row/5.7706 cache is a different snapshot, not changed model behavior. Default ordinary/conditional gates and model parameters are unchanged; returning-history and kicking-events-only baselines require separate opt-in and preserve scoring/availability limitations. This run does not validate these new cohorts or custom scoring; their prior exploratory evidence and exact release scope are in `docs/coverage-evaluation.md`. No 2025 evaluation or published-metric rewrite. |
