...
| September 9, 2026 — current-week waiver membership and saved-team workflow | `pnpm backtest` | Unchanged model: development MAE 5.8818 over 26,837 rows; tuning 5.7709 over 9,063 rows on the shared operator cache (the previously documented 9,069-row cache has 5.7706). The waiver filter/UI are outside this baseline code path. No 2025 holdout evaluation or published-metric rewrite. Membership and real-worker synthetic comparison tests establish scope/assignment behavior, not forecast accuracy or a waiver edge. |
