...
Changes that are required to re-run the backtest but do not move it are recorded here
rather than left implicit. "No figure moved" is a result, and one a reader can only trust if
somebody wrote down that the command was run.

| Change | Command | Result |
| --- | --- | --- |
| September 9, 2026 — explicit experimental weekly UI consent and provenance | `pnpm backtest` with the same frozen source cache as the producer | Development MAE 5.8818 (26,837 rows) and tuning 5.7709 (9,063 rows), unchanged. The UI requests and selects separate default-off experimental baselines only with explicit consent, preserves raw missing/manual values and saves the experimental origin and preference. Ordinary estimates and model parameters are unchanged. This run does not validate the opt-in returning-history/kicking-event cohorts, custom scoring, sensitivity thresholds or real-world lineup improvement. Holdout 2025 not evaluated; published metrics untouched. |
| September 9, 2026 — explicit experimental week-1 coverage alternatives | `pnpm backtest` on base `5d0c484` and candidate with identical frozen source cache | Both reproduce development MAE 5.8818 (26,837 rows) and tuning 5.7709 (9,063 rows). The earlier fresh-source 9,069-row/5.7706 cache is a different snapshot, not changed model behavior. Ordinary history/injury guards and model parameters are unchanged; contradictory current-week/season identity bridges now fail closed. Returning-history and kicking-events-only baselines require separate opt-in and preserve scoring/availability limitations. This run does not validate these new cohorts or custom scoring; their prior exploratory evidence and exact release scope are in `docs/coverage-evaluation.md`. No 2025 evaluation or published-metric rewrite. |
| September 9, 2026 — authenticated bounded football-data reads | `pnpm backtest` | Development/tuning MAE remains 5.8818 / 5.7706 on the existing 26,837/9,069-row cache. The change protects reads and rejects oversized whole pools instead of silently truncating them; signed-in numeric results and ranking below the budgets are unchanged. No model retuning, 2025 holdout evaluation or published-metric rewrite. |
| September 9, 2026 — saved personal workflow and bounded private journal | `pnpm backtest` with freshly downloaded historical inputs | Development MAE 5.8818 over 26,837 rows and tuning 5.7706 over 9,069 rows. This fresh-source cache differs from the foundation's frozen 9,063-row snapshot below; no model parameters or published metrics changed. Authentication, storage, presentation and exact-input persistence are workflow changes, not new predictive evidence. Reserved 2025 holdout not evaluated. |
| September 9, 2026 — bounded one-week add/drop comparison | `pnpm backtest` | Development model MAE 5.8818 and tuning 5.7706, unchanged. Holdout 2025 not evaluated; published metrics untouched. The projection model is unchanged. Independent small-case enumeration verifies marginal exact included-lineup assignment against the optimized existing roster; full-league ownership, kickoff locks and fixed-unpriced-scope tests verify decision boundaries. This does not establish waiver outcome accuracy, season-long drop value or superiority over another strategy. See `docs/waivers.md`. |
| September 9, 2026 — visible draft evidence and promotion-review gate | `pnpm backtest` | Development/tuning MAE remains 5.8818 / 5.7706; 2025 holdout not evaluated and published metrics untouched. The versioned gate drives the draft evidence status and fails closed on missing independent preregistered comparisons, roster/missingness audits or browser/mobile latency evidence. Current descriptive reused cases remain not promoted. No selection, ranking, model parameter or default-policy change; the in-app summary reports the existing draft evaluation, not new accuracy evidence. |
| September 9, 2026 — prospective decision journal and unknown-kickoff safety | `pnpm backtest` on base `a48dbdb` and candidate with the same cache | Both produce development MAE 5.8818 (26,837 rows) and tuning 5.7709 (9,063 rows), unchanged by this code. This cache differs from the earlier run's 9,069-row/5.7706 tuning snapshot; it is not a new model improvement or a rewrite of published evidence. Missing or malformed schedule times now remain unknown instead of becoming midnight. Journal evaluation freezes original/recommended lineups, independently checks kickoffs, and waits for closed periods plus posted game results. Holdout 2025 not evaluated; published metrics untouched. |
| September 9, 2026 — current-week waiver membership and saved-team workflow | `pnpm backtest`, rerun after identity reconciliation | Unchanged model: development MAE 5.8818 over 26,837 rows; tuning 5.7709 over 9,063 rows on the shared operator cache (the previously documented 9,069-row cache has 5.7706). The waiver filter/UI are outside this baseline code path. No 2025 holdout evaluation or published-metric rewrite. Membership, real-adapter identity-conflict regressions and real-worker synthetic comparisons establish scope/assignment behavior, not forecast accuracy or a waiver edge. |
| September 9, 2026 — source-coverage transparency and fixed experimental coverage audit | `pnpm backtest`; `pnpm coverage-audit` | Core development/tuning MAE remains 5.8818 / 5.7706; 2025 holdout not evaluated and published metrics untouched. Separate conditional-on-appearance K/DST and limited/returning-history measurements are recorded in `docs/coverage-evaluation.md`, not promoted to production projections. Injury warnings expose observed team/report/date scope without claiming complete reporting or changing eligibility gates. |
| September 9, 2026 — immediate market comparator and incomplete-opponent disclosure | `pnpm backtest` | Development MAE 5.8818 and tuning 5.7706, unchanged. Holdout 2025 not evaluated; published metrics untouched. UI exposes an immediate lowest-ADP option under the existing completion guard, independently of simulation, without changing the production rollout ranking or claiming a drafting edge. |
| September 9, 2026 — opt-in conditional-on-active weekly estimates | `pnpm backtest` | Development/tuning model MAE remains 5.8818 / 5.7706 with the unchanged model. Holdout 2025 not evaluated; published metrics untouched. The optional separately typed value retains all identity, roster, injury designation, game, scoring and history gates and does not change ordinary points or claim availability-adjusted accuracy. Regression tests cover default-off behavior, forbidden cases, and signed conditional values. |
| September 9, 2026 — required draft starters and paired strategy evaluation | `pnpm backtest` | Development model MAE 5.8818 vs prior-mean 6.0099; tuning 5.7706 vs 5.9453, unchanged. Holdout 2025 not evaluated; published metrics untouched. Structural draft feasibility and the separate strategy evaluation do not change the weekly projection model or establish calibrated title odds. See `docs/draft-strategy-evaluation.md`. |
| September 9, 2026 — personal nflverse weekly estimate adapter | `pnpm backtest` | Development model MAE 5.8818 vs prior-mean 6.0099; tuning 5.7706 vs 5.9453 over 9,069 player-weeks. Holdout not evaluated and published metrics untouched. The adapter uses the unchanged model under the supported subset of imported offensive scoring; custom scoring and omitted special-teams terms have no new accuracy validation. Tests cover temporal history exclusion, identity, availability and explicit partial-scoring provenance. Follow-up checks retain signed estimates, corrected injury designations, active transaction destinations and missing-line warnings. Inactive identities, refusal of penalty-only or unrelated-position scoring, current game context and missing target-week injury coverage are also covered; the default backtest remains unchanged. |
| September 9, 2026 — offline Sleeper projection and ADP contracts | `pnpm backtest` | Development model MAE 5.8818 vs prior-mean 6.0099; tuning 5.7706 vs 5.9453 over 9,069 player-weeks. Holdout 2025 not evaluated; published metrics untouched. The new parser/scorer is not used by this baseline run. Strict fractional-scoring and missing-input tests validate the new source contract; they do not establish prediction accuracy. |
...
| September 8, 2026 — signed custom outcomes and traded draft capacity | `pnpm backtest` | Development and tuning results unchanged (5.8818 / 5.7706 model MAE); holdout untouched. New simulation tests retain negative starts, choose signed-position starters before outcomes, check measured-mean convergence and memo isolation, and consume extra traded picks before roster cuts. These changes are not evaluated by the baseline projection backtest and do not establish real-world title-probability calibration. |
| September 8, 2026 — zero-inclusive custom skill outcomes | `pnpm backtest` | Development model MAE 5.8818 vs prior-mean 6.0099; tuning 5.7706 vs 5.9453, unchanged; 2025 holdout not evaluated. Live custom history exposed RB/WR/TE p10 = 0, which is incompatible with the legacy lognormal fit. Custom skills now sample 100 measured, mean-normalized empirical ratio knots and use pre-game lineup selection. Seeded tests verify zero mass, bounded outcomes, negative draws, mean convergence and cache isolation. Full verification passed 83 files / 1801 tests before this documentation update. This is numerical/source validation, not a measured advantage over ADP or calibrated title odds. |
...
`scripts/backtest.ts` shares no code path with `lib/core/draft-policy.ts`. The run is
evidence of exactly that independence. What the draft *board* claims about ranking players
is measured separately, by `pnpm draft-backtest`, and lives in
[`docs/draft-validation.md`](draft-validation.md).
