...
const sharedFetcher = createSleeperLineupFetcher(httpTextFetcher, () => Date.now());
...
export function parseSleeperLineup(leaguePayload: unknown, rostersPayload: unknown, playersPayload: unknown, request: { leagueId: string; ownerId: string; week: number; now: number }): WeeklyLineupSnapshot {
  ...
  const players = ids.map((id) => {
    ...
    if (reserve.has(id)) availability = "reserve";
    ...
    return { id, name, positions, availability, kickoffAt: null, currentSlotId: starters.includes(id) ? slots[starters.indexOf(id)].id : null, projectedPoints: null };
  });
...
export async function importSleeperLineup(request: { leagueId: string; ownerId: string; week: number; now: number }, fetchText: TextFetcher = sharedFetcher): Promise<WeeklyLineupSnapshot> {
